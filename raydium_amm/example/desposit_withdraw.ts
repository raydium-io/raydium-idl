...
(async () => {
  ...
  const ammProgram = raydiumAmmProgram({
    provider: provider,
    programId: new PublicKey("AMMjRTfWhP73x9fM6jdoXRfgFJXR97NFRkV8fYJUrnLE"), // devnet amm program
  });
  ...
  const lpMint = new PublicKey(accounts.lpMint);
  const userCoinTokenAccount = getAssociatedTokenAddressSync(
    coinMint,
    wallet.publicKey
  );
  const userPcTokenAccount = getAssociatedTokenAddressSync(
    pcMint,
    wallet.publicKey
  );
  ...
  const tx = await ammProgram.methods
    .deposit(new BN(10000), new BN(10000), new BN(0))
    .accounts({
  ...
  const tx2 = await ammProgram.methods
    ...
    .accounts({
      ...
      poolCoinTokenAccount: new PublicKey(accounts.baseVault),
      poolPcTokenAccount: new PublicKey(accounts.quoteVault),
      poolWithdrawQueue: new PublicKey(accounts.withdrawQueue),
      poolTempLpTokenAccount: new PublicKey(accounts.lpVault),
      lpMintAddress: lpMint,
      ...
      serumEventQ: new PublicKey(accounts.marketEventQueue),
